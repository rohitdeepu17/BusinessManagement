...
$qry = "SELECT vendor_name, address, balance FROM vendor ORDER BY address;";
$total_balance = 0.00;
...
while($data = mysqli_fetch_assoc($res)){
   ...
   $this->Cell($w[2],$h,number_format($data['balance']),'LR',0,'R', $fill);
   //separation line
   $this->Ln();
...
$pdf = new PDF();
// Column headings
$header = array('Vendor Name', 'Address', 'Balance');
// Data loading
$pdf->SetFont('Arial','',10);
$pdf->AddPage('L');
$pdf->ImprovedTable($header, $conn);
//Need to look better logic to take understandable date/time
$pdf->Output("", "VendorsBalance_".date("Y-m-d",time()).".pdf", false);
?>